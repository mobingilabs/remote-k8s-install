...
func CommandList(cfg *config.Config) (machine.CommandList, error) {
	...
	writeCmd := cmdutil.NewWriteCmd(filepath.Join(constants.ServiceDir, constants.KubeSchedulerService), string(serviceData))
	writeCheck := func(output string) bool {
		return true
	}
	cl.Add(writeCmd, writeCheck)

	startCmd := cmdutil.NewSystemStartCmd(constants.KubeSchedulerService)
